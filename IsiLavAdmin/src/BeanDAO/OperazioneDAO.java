...
public class OperazioneDAO {
	public synchronized ArrayList<Percorso> prenotazioni(String data, String giorno) { 
		...
		try {
			...
			switch(giorno) {
			case "lunedì":
				ps = (PreparedStatement) conn.prepareStatement("SELECT DISTINCT operazione.id_percorso, percorso.nome FROM operazione join fermata on operazione.id_percorso=fermata.id_percorso and operazione.via= fermata.via join percorso on fermata.id_percorso= percorso.id_percorso where lunedì=1 and data_scelta=? ORDER BY nome ASC ");
				ps.setString(1, data);
				break;
			case "martedì":
				ps = (PreparedStatement) conn.prepareStatement("SELECT DISTINCT operazione.id_percorso, percorso.nome FROM operazione join fermata on operazione.id_percorso=fermata.id_percorso and operazione.via= fermata.via join percorso on fermata.id_percorso= percorso.id_percorso where martedì=1 and data_scelta=? ORDER BY nome ASC ");
				ps.setString(1, data);
				break;
			case "mercoledì":
				ps = (PreparedStatement) conn.prepareStatement("SELECT DISTINCT operazione.id_percorso, percorso.nome FROM operazione join fermata on operazione.id_percorso=fermata.id_percorso and operazione.via= fermata.via join percorso on fermata.id_percorso= percorso.id_percorso where mercoledì=1 and data_scelta=? ORDER BY nome ASC ");
				ps.setString(1, data);
				break;
			case "giovedì":
				ps = (PreparedStatement) conn.prepareStatement("SELECT DISTINCT operazione.id_percorso, percorso.nome FROM operazione join fermata on operazione.id_percorso=fermata.id_percorso and operazione.via= fermata.via join percorso on fermata.id_percorso= percorso.id_percorso where giovedì=1 and data_scelta=? ORDER BY nome ASC");
				ps.setString(1, data);
				break;
			case "venerdì":
				ps = (PreparedStatement) conn.prepareStatement("SELECT DISTINCT operazione.id_percorso FROM operazione join fermata on operazione.id_percorso=fermata.id_percorso and operazione.via= fermata.via join percorso on fermata.id_percorso= percorso.id_percorso where venerdì=1 and data_scelta=? ORDER BY nome ASC ");
				ps.setString(1, data);
				break;
			case "sabato":
